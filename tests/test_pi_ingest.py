...
def test_process_once_infers_pi_model_from_provider_payload(tmp_path: Path) -> None:
    ...
    log_path.write_text(
        json.dumps(
            {
                "at": "2026-05-03T06:12:01.000Z",
                "level": "debug",
                "message": "suggestion.provider.payload",
                "meta": {"payloadPreview": json.dumps({"model": "gpt-5.5", "stream": True})},
            }
        )
        + "\n"
        + json.dumps(
    ...
    assert counts["pi_records"] == 2
    assert sink.captured[1].tags["agent_model"] == "gpt-5.5"
    assert sink.captured[1].measurements["cost_usd"] == 0.027015
